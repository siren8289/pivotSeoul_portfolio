...
Life Pivot AI 서버. FastAPI 기반이며, **기능 도메인별 폴더**(`lifePivot_/app/modules/{기능}/`) 안에 HTTP 계약(`router`, `schema`)과 **같은 기능의 파이프라인**(`pipelines/`)을 둔다.
...
2. `uvicorn main:app --reload --port 8000` (현재 디렉터리가 `ai`일 때)
