          ...
          return (
            <div key={s.label} className="flex items-center flex-1">
              <div className="flex flex-col items-center gap-1">
                <div className="w-8 h-8 rounded-full flex items-center justify-center transition-all duration-300" style={{ background: isDone ? c.success : isActive ? c.primary : c.badgeBg, color: isDone || isActive ? 'white' : c.textMuted, boxShadow: isActive ? `0 0 15px ${c.primaryBg}` : 'none' }}>
                  {isDone ? <CheckCircle2 size={14} /> : <Icon size={14} />}
                </div>
                <span style={{ color: isActive ? c.primary : isDone ? c.success : c.textMuted, fontSize: '0.68rem' }}>{s.label}</span>
              </div>
